...
Now let's try to understand what is there inside the JVM. JVM is where you execute your byte code right? Now the question is how exactly are we going to execute the code? Of course there is something called a runtime or the runner which will do that. Here we will focus more on the memory level.
